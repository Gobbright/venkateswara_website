...
import { ChevronDown, Gem, ShoppingBag, ShoppingCart, Star, Watch } from "lucide-react";
import { getShopItems, toggleShopItem } from "../utils/shopItems";
import mensBelt from "../assets/Images/Mens/Accessories/belt.jpg";
import womensAccessories from "../assets/Images/Womens/Accessories/Ac.jpg";

const categories = [
  { category: "Mens Accessories", image: mensBelt, basePrice: 299, icon: Watch },
  { category: "Womens Accessories", image: womensAccessories, basePrice: 399, icon: Gem },
  { category: "Kids Accessories", image: mensBelt, basePrice: 199, icon: ShoppingBag },
];
...
const products = Array.from({ length: 18 }, (_, index) => {
  ...
  return {
    ...
    oldPrice: item.basePrice + index * 80 + 350,
    reviews: 45 + index * 9,
    isBestseller: index < 3,
    slug: `accessories-${item.category.toLowerCase().replaceAll(" ", "-")}-${index + 1}`,
  };
});
  ...
  return (
    <div className="min-h-screen bg-[#f5ede3] p-3 sm:p-4 md:p-6">
      <div className="mb-3 text-center">
        <h1 className="text-2xl font-semibold md:text-4xl">Accessories Collection</h1>
        <p className="mt-1 text-sm text-gray-600 md:text-base">
          Browse all accessories in one page
        </p>
      </div>
            ...
            <div className="flex min-h-0 flex-1 flex-col px-3 py-3 sm:px-5 sm:py-4">
              ...
              <div className="mt-auto grid grid-cols-2 gap-1.5 pt-4 sm:gap-2">
                <button
                  ...
                  className={`flex h-9 items-center justify-center gap-1 rounded-full border px-1 text-[11px] font-bold transition sm:h-10 sm:px-3 sm:text-sm ${
